...
def test_training_contract_matches_the_build_script(contract, sparkov_build) -> None:
    ...
    assert {
        contract["train_partition"],
        contract["calibration_partition"],
        contract["test_partition"],
    } == set(sparkov_build.PARTITIONS)
...
def test_training_contract_keeps_the_no_authority_boundary(contract) -> None:
    ...
    assert any(
        "No score can approve" in boundary for boundary in contract["release_boundary"]
    )
...
def test_committed_report_agrees_with_the_contract(contract, report) -> None:
    ...
    assert (
        report["input_manifest"]["release_criteria_version"]
        == contract["release_criteria_version"]
    )
...
def test_notebook_pipeline_lists_every_notebook_in_order(repository_root) -> None:
    ...
    on_disk = sorted(
        path.name for path in (repository_root / "notebooks").glob("[0-9][0-9]-*.ipynb")
    )
