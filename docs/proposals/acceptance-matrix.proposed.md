...
| FR-05 bounded investigation | ADR-006/008/014–017; accepted public-showcase HTTP/SSE, fixture and safeguard artifacts | Exact scenario-only request and HTTP errors, ordered S01/S04/S05 transcripts, loader rejection, deterministic bypass, three-tool allowlist and rejection, call budget, same-run citations, stable incomplete failures, process-local admission, redaction and authority non-bypass tests; later operational tests | Local S01–S05 API runtime implemented; browser, Azure and full Phase 3 pending | MVP 3 bounded slice / F4 |
...
| `tests/test_proposed_showcase_investigation_controls.py` | Accepted live mode defaults off; exact access limits; Groq-only server credential/model policy; validated output/no-raw-logging boundary; no second-LLM fallback; local-only legacy coexistence; four cutover gates; explicit cutover; active-code/dependency removal with characterization/history preservation | Configured model choice, actual migration evidence and cutover decision, trusted Azure ingress parsing, provider spending enforcement or durable quota |
| `tests/test_public_showcase_api_contract.py` | Accepted scenario-only request and redacted HTTP errors; ordered terminal SSE semantics; S01 skip, S04 two-tool grounded result and S05 no-provider incomplete failure; no caller prompt/model, runtime score, threshold or action | Runtime behavior, live-provider quality, browser consumption or deployment |
| `tests/test_showcase_investigation_runtime.py` | Accepted fixture/config loading; S01–S03 deterministic bypass; exact S04 playback; S05 incomplete outage; deferred S06–S08; redacted validation; bounded LangGraph tools/citations/failures; Groq adapter shape; process-local admission | Live-provider quality, trusted Azure ingress, browser consumption, durable quota, operational F3/F4 behavior or public deployment |
