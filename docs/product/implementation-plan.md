...
| MVP 3 — deployed public showcase | `SHOWCASE_WEB_URL=… SHOWCASE_API_URL=… make acceptance-mvp3-public` | Record the public URL, health/cold-start/unavailable/retry journey, explicit CORS grant and foreign-origin rejection, deployment architecture/runbook, budget-alert configuration and delivery status, and teardown owner. |

The MVP 3 public command checks public health and CORS only; it does not deploy,
inject secrets, or substitute for a manual public-demo journey. It passed
against the recorded-only Azure deployment on 2026-09-21, alongside a manual
Chrome journey recorded in the release evidence.
...
| --- | --- | --- | --- | --- |
| ✓ | Overview | `/overview` | Shared app shell, header scenario selector, first-run zero state, guided demo, mock outcome mix, KPI strip, operational-health state, quick actions, and recent decisions. The header selector lists the same runnable S01–S05 scenarios as Showcase investigation, and its Run opens that page with the scenario selected and runs it as recorded playback; it does not fill the dashboard. The dashboard figures come only from the representative mixed portfolio, loaded from the empty-state card, and remain zero until then. All demo content is labelled as representative data. The Overview header and the Payments shell top bar still differ by design until a shell decision is approved. | `dashboard.spec.ts` (including the header-to-investigation hand-off with a stubbed API), `welcome.spec.ts`, `tour.spec.ts`, axe in `accessibility.spec.ts`, and route and theme-scope checks in `payments-design-system.spec.ts`; `make acceptance-mvp1`. The hand-off is not yet covered against the real local API. |
...
| --- | --- | --- | --- |
| ✓ | Web console | Azure Static Web Apps hosts the synthetic React/Vite console. The API base URL is public configuration, never a secret. | Deployed on Static Web Apps Free and verified at the public URL by workflow run `35611121637` and a manual Chrome journey; see the 2026-09-21 release evidence. |
| ✓ | Demo API | Azure Container Apps Consumption hosts the FastAPI demo with minimum replicas of zero. The UI exposes health, connection, cold-start, unavailable, and retry states. | Public health passed; the deployment is 0–1 replicas with HTTPS-only ingress. Local browser tests cover slow wake-up, unavailable, and retry states. |
| ✓ | Public-safe investigation | Replace the public image's unavailable private-SDK run path with a repository-owned, scenario-only, bounded LangGraph investigation or its clearly labelled recorded demonstration playback. S01–S03 visibly emit an investigation-skipped event; S04 uses the accepted payee/device evidence tools, plus a third, live-only `get_account_activity_evidence` tool (ADR-018/019) carrying one Plaid Sandbox-derived evidence item, clearly labelled and never part of the fixed recorded script; the budget is three total calls and one per tool; S05 injects a no-cost deterministic outage. Every provider/tool/output/timeout/budget failure remains `incomplete`, recommends fail-safe HOLD, leaves authority unevaluated and executes nothing. MVP 3 has no numeric runtime fraud-model score or threshold. | Public S04 and S05 API traces and their browser presentation passed on 2026-09-21. The Plaid-derived S04 account-activity path was verified against the real local API and a live Groq run, and against `test_showcase_investigation_runtime.py`, `test_f3_preparation_artifacts.py`, and `showcase-investigation.spec.ts` on 2026-09-22. The client validates event shapes, one-run identity, sequence and terminal-result ordering. Optional live-provider evaluation remains deliberately unclaimed. |
| ✓ | Delivery | GitHub Actions validates builds/tests, then deploys through Azure OIDC. Bicep and a deployment runbook make the release reproducible. | Protected workflow run `35611121637` passed the pre-deploy gate, immutable image publication, OIDC authentication, `what-if`, deployment, Static Web Apps publication and public acceptance. |
| ◐ | Cost/safety | Doppler holds server-side secrets; no database, cache, VNet, real payment, or customer/provider data is deployed. Recorded playback is continuously public; anonymous live Groq defaults off. A controlled window permits one concurrent run, two per observed client per 10 minutes, ten per process, 30 minutes maximum and a 45-second overall timeout. A budget alert and free-grant review are documented. | The recorded-only deployment has no provider secret or approved model, live mode is false, scale is 0–1, and monthly 80%/100% alerts target the monitored mailbox. Alert delivery is unexercised because no threshold has been crossed; Doppler remains optional until a live model is approved. |
| ◐ | Screens | MVP 1–2 screens remain available; the console gains explicit recorded/live labelling and the bounded S04/S05 evidence trace without adding review mutations. Not Found and the frozen reference route remain unchanged. | Public Chrome verification passed for Overview and S04/S05 with no browser errors. Automated tests cover the other states and WCAG A/AA; manual keyboard and screen-reader review remains. |
...
| --- | --- | --- | --- |
| Budget alerts notify but do not cap spend; delivery has not been exercised because no 80%/100% threshold has been crossed. | MVP 3 cost control | Platform/release owner | Review Azure cost regularly and record the first delivered alert if a threshold is crossed. |
| The private Arbiris SDK remains excluded from the verified public image; passing the public SDK-free path does not itself approve removal of the local compatibility workflow. | Legacy cutover | API and product owners | Record the explicit cutover decision in `docs/proposals/public-showcase-investigation.proposed.md`. |
...
items checked (manual assistive-technology review remains), MVP 2 has 6 of 6,
and MVP 3 has 9 of 12 items checked plus 3 partial items. The Azure release is
live and verified; the partial rows preserve optional provider/Doppler work,
untriggered budget-alert delivery, the existing-tour status gap, and manual
assistive-technology review without disguising them as completed evidence.
...
| ✓ | Add GitHub Actions deployment through Azure OIDC. | Cloud IAM and CI/CD | Automated gates plus verified cloud execution | `.github/workflows/deploy-showcase.yml` is manual, environment-protected, restricted to `main`, SHA-pins every action, publishes an SDK-free GHCR image by immutable digest, uses OIDC with no Azure client secret, runs `what-if`, retrieves the SWA token only at runtime, and executes public acceptance. Run `35611121637` passed on commit `942f92b`; the service principal has Contributor only on the dedicated resource group. |
| ◐ | Configure Doppler `showcase` values and host-side secret injection. | Secrets management | Injection path verified statically; optional provider values deliberately absent | The official pinned fetch action accepts an optional read-only `DOPPLER_TOKEN`; Bicep exposes the Groq key only as a Container Apps secret reference and never to Vite. The verified recorded-only deployment intentionally has no Doppler token, provider key, or model. No model identifier has been approved, no value has been invented, and `SHOWCASE_LIVE_ENABLED=false`. |
| ✓ | Define Container Apps scale-to-zero and no-extra-services configuration. | Serverless operations and cost control | Configuration and deployed-resource inspection | Azure reports 0–1 replicas, 0.25 vCPU/0.5 GiB, public HTTPS-only ingress, the exact web origin in `ALLOWED_ORIGINS`, and no database, cache, VNet, or logs workspace. |
| ◐ | Prepare a budget-alert configuration. | FinOps and cloud governance | Deployed configuration verified; alert delivery not triggered | The monthly budget is configured at 10 subscription currency units with enabled 80% and 100% email alerts to `accounts@arbiris.uk`. Alerts notify but do not cap consumption. Delivery is not claimed because no threshold has been crossed. |
| ✓ | Deploy both applications and set explicit API allowed origins. | Cloud deployment | Protected workflow plus public smoke and browser journey | Static Web Apps Free and the scale-to-zero Container App were deployed by run `35611121637`. Public health passed, the SPA fallback worked on `/transactions/investigation`, the UI reported the API ready, and Chrome completed S04 and S05. |
| ✓ | Verify API error redaction and cross-origin configuration against the public-showcase environment. | Application security and CORS | Automated workflow plus independent public requests | The deployed origin receives `access-control-allow-origin`; a foreign origin receives no grant. An invalid investigation returned only the stable `invalid_request` code and message. S05 exposed the stable provider-unavailable presentation without raw provider detail. |
| ✓ | Publish Azure architecture, runbook, teardown steps, and a public-demo smoke-test result. | Technical documentation and operations | Runbook plus dated release evidence | `infra/azure/README.md` records bootstrap, release, rollback, and teardown. `docs/audits/2026-09-21-mvp3-azure-release.md` records run `35611121637`, commit, immutable digest, URLs, regions/plans, IAM, budget, automated checks, Chrome results, residual items, and the resource-group owner teardown path. |
