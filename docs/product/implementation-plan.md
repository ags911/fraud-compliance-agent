...
| Status | Completion item | Evidence / next action |
| --- | --- | --- |
| ✓ | Overview has a first-run zero state, persistent scenario control, guided getting-started flow, and synthetic-data labelling. | `apps/web/src/Overview.tsx`. |
...
| — | Perform and retain screenshot/accessibility checks for every MVP 1 route at desktop and narrow widths. | Run after final integration styling. |
