...
| ◐ | Public-safe investigation | Replace the public image's unavailable private-SDK run path with a repository-owned, scenario-only, bounded LangGraph investigation or its clearly labelled recorded demonstration playback. S01–S03 visibly emit an investigation-skipped event; S04 uses the accepted payee/device evidence tools; the budget is three total calls and one per tool; S05 injects a no-cost deterministic outage. Every provider/tool/output/timeout/budget failure remains `incomplete`, recommends fail-safe HOLD, leaves authority unevaluated and executes nothing. MVP 3 has no numeric runtime fraud-model score or threshold. | API side complete under ADR-017: validating loader, deterministic routes, playback, bounded graph, Groq adapter, admission controls, contract tests and container smoke check. Browser consumer, evaluation UX and public deployment checks remain. |
...
| ◐ | Screens | MVP 1–2 screens remain available; the console gains explicit recorded/live labelling and the bounded S04/S05 evidence trace without adding review mutations. Not Found and the frozen reference route remain unchanged. | Built locally at `/transactions/investigation`, a separate surface so the legacy A–F workspace stays until an explicit cutover decision. `showcase-investigation.spec.ts` covers the S04 evidence trace and same-run citations, the S01 investigation-skipped trace, the S05 incomplete fail-safe path, live provider/model labelling, the redacted deferred-scenario message, and refusal to report completion without the terminal event; the route is in the WCAG A/AA sweep and has a post-run accessibility check. Public-URL verification and the manual keyboard/screen-reader review remain. |
...
| The private Arbiris SDK cannot be placed in the public container image; its SDK-free API replacement is built but not connected to the browser or verified on Azure. | MVP 3 live-run surface | API and product owners | Complete browser acceptance, public-container execution and the explicit legacy cutover gate in `docs/proposals/public-showcase-investigation.proposed.md`. |
...
has 4 of 13 completed/configuration-reviewed items plus 1 partial configuration item. Several
...
| ◐ | Add an API status step to the guided tour for the health and cold-start state. | Reliability UX | Automated browser tests | Built 2026-09-21. `useApiHealth` probes `/health` and the sidebar reports checking, waking, ready or unavailable, replacing hard-coded "All systems operational" text. The showcase investigation tour opens on that status and explains cold start; `showcase-explain.spec.ts` covers all three states including a slow probe. **Gaps:** the connection, cold-start and unavailable states are shown but no retry control is exposed, so the retry state is unbuilt (`recheck()` exists but nothing calls it); and this is a new per-page tour on the showcase route rather than an extension of the existing tours, which still have no status step. |
